...
const ManageHospitals = () => {
  ...
  const validate = () => {
    ...
    if (!/^[0-9]{10}$/.test(updatedData.number))
      tempErrors.number = "Contact number must be 10 digits";
    if (!updatedData.specialization)
      tempErrors.specialization = "Specialization is required";
    // if (!updatedData.capacity || isNaN(updatedData.capacity))
  ...
  const handleAddHospital = () => {
    navigate("/register-hospitals");
  };

  return (
    <div className="flex">
      <SideBar />
      <div className="max-w-2xl mx-auto p-4 flex-1">
        {" "}
